...
var scanCmd = &cobra.Command{
	...
	Long: ui.RenderCommandHelp("scan",
		...
		`  vg scan                            Scan current directory
  vg scan .                           Scan current directory
  vg scan /path/to/project            Scan a specific path
  vg scan fs /path/to/project         Scan with explicit fs subcommand
  vg scan repo https://github.com/org/repo  Scan a Git repository`),
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return runScanFS(target)
	},
}

func runScanFS(target string) error {
	...
	return runPrettyScan(target, "filesystem", trivyArgs)
}
